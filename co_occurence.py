...
def computeCoOccurrenceMatrix(corpus, window_size=4):
    ...
    corpus_words, num_corpus_words = distinctWords(corpus)
    ...
    for sublist in corpus:
        for index, item in enumerate(sublist):
            for shift in range (window_size * -1, window_size +1):
                if shift == 0:
                    continue
                if index + shift < 0:
                    continue
                if index + shift >= len(sublist):
                    continue
                M[word2Ind[sublist[index + shift]],word2Ind[sublist[index]]]+=1
